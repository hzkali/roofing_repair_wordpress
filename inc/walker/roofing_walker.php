 ...
 * ---------------menu--------------------------------
 */ 
class roofing_top_bar_walker extends Walker_Nav_Menu {
	...
	function start_el(&$output, $item, $depth = 0, $args = Array(), $id = 0) {
	  ...
	  if(is_object($args)){
		  $roofing_class = '';
			...
			$roofing_icon = $item->mega_menu_icon ;
		if($item->mega_menu == 1) {
			$roofing_class= 'wd_mega-menu';
		}
			...
			$class_names = ($args -> has_children) ? 'has-dropdown not-click '.$roofing_class : '';
			...
			$attributes .= ($args -> has_children ) ? ' class=" toggle has-icon" ' : ' class="has-icon"';

			$item_output = $args -> before;
			$item_output .= (!in_array('section', $classes)) ? '
			<a' . $attributes . '>' : '';
			if(($roofing_icon != '') and ($roofing_icon != '---- None ----'))  {
				
			$item_output .= '<i class="'.$roofing_icon.' fa"></i> ';
			}
...
if ( !function_exists( 'roofing_add_mega_menu_support' ) ):

	function roofing_add_mega_menu_support() {


			add_filter( 'wp_setup_nav_menu_item', 'roofing_add_custom_nav_fields' );

			add_action( 'wp_update_nav_menu_item', 'roofing_update_custom_nav_fields', 10, 3 );

			add_filter( 'wp_edit_nav_menu_walker', 'roofing_edit_menu_walker', 10, 2 );
...
add_action( 'init', 'roofing_add_mega_menu_support' );



/* Add custom fields to menu */

if ( !function_exists( 'roofing_add_custom_nav_fields' ) ):

	function roofing_add_custom_nav_fields( $menu_item ) {

		
		$menu_item->mega_menu_icon = get_post_meta( $menu_item->ID, '_roofing_mega_menu_icon', true )? get_post_meta( $menu_item->ID, '_roofing_mega_menu_icon', true ) : '';
		
		$menu_item->mega_menu_bg_image = get_post_meta( $menu_item->ID, '_roofing_mega_menu_bg_image', true )? get_post_meta( $menu_item->ID, '_roofing_mega_menu_bg_image', true ) : '';
		
		$menu_item->mega_menu = get_post_meta( $menu_item->ID, '_roofing_mega_menu', true ) ? 1 : 0;
...
if ( !function_exists( 'roofing_update_custom_nav_fields' ) ):

	function roofing_update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ) {


			$value = isset($_REQUEST['menu-item-mega-menu-icon'][$menu_item_db_id])? $_REQUEST['menu-item-mega-menu-icon'][$menu_item_db_id] : '' ;
			update_post_meta( $menu_item_db_id, '_roofing_mega_menu_icon', $value );
			
			
			$value = isset($_REQUEST['menu-item-mega-menu-bg-image'][$menu_item_db_id])? $_REQUEST['menu-item-mega-menu-bg-image'][$menu_item_db_id] : '' ;
			update_post_meta( $menu_item_db_id, '_roofing_mega_menu_bg_image', $value );
		

			$value = isset( $_REQUEST['menu-item-mega-menu'][$menu_item_db_id] ) ? 1 : 0;
			update_post_meta( $menu_item_db_id, '_roofing_mega_menu', $value );
...
if ( !function_exists( 'roofing_edit_menu_walker' ) ):

	function roofing_edit_menu_walker( $walker, $menu_id ) {



		class roofing_Walker_Nav_Menu_Edit extends Walker_Nav_Menu_Edit {
				...
				$mega_menu_html = '';
  				...
  				$icons = unserialize( $icons );
		        	...
		        	$mega_menu_html .= '</select>
		                ...
		                '.__( 'display icone', 'roofing' ).'</label>
					...
					$mega_menu_html .= '<p class="field-custom description description-wide">
			                ...
			                '.__( 'Mega Menu (Background image)', 'roofing' ).'
		...
		return 'roofing_Walker_Nav_Menu_Edit';
